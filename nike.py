...
class Nike:
    ...
    def login(self, login_url:str) -> None:
        '''Login in nike.com'''
        self.open_url(url=login_url, open_tab=True)
        self.enter_write_edittext('password', self._dir, self.password)
        self.enter_write_edittext('email', self._dir, self.email)
        self.enter_button('sign_in', self._dir)
    
    def open_json(self, _dir_json:str) -> json.JSONDecoder | None:
        try:
    ...
    def write_json(self, _dir_json:str, upload_json:dict) -> None:
        try:
    ...
    def square_date(self, date_no_parse:str) -> bool:
    ...
    def cart(self, url_cart:str) -> None:
        '''Go cart and confirm buy'''
        self.open_url(url=url_cart, select_url=True, backspace=True)
        self.enter_button(name='member_checkout', _dir=self._dir, upload_url=True)
        self.enter_button(name='continue', _dir=self._dir, upload_url=True)
        self.enter_button(name='arrives', _dir=self._dir)
        self.enter_button(name='continue', _dir=self._dir, upload_url=True)
        self.enter_button(name='continue1', _dir=self._dir, upload_url=True)
        self.enter_write_edittext(name='cvv', _dir=self._dir, text=self.cvv)
        self.enter_write_edittext(name='mmyy', _dir=self._dir, text=self.mmyy)
        self.enter_write_edittext(name='card_number', _dir=self._dir, text=self.card_number)
        self.enter_write_edittext(name='name_on_card', _dir=self._dir, text=self.name_on_card)
        self.enter_button(name='place_order', _dir=self._dir)

    def buy(self, data_json:str, shoe_size:str, url_cart:str, screenshot:bool=False) -> bool:
        download_json = self.open_json(_dir_json=path.join(self._dir_config, data_json))
        ...
        while upload_json:
            ...
            self.write_json(_dir_json=path.join(self._dir_config, data_json), upload_json=upload_json)


    def opencv_element(self, name_element: str) -> tuple | str:
        while True:
            self.restart_url()
    ...
    def enter_button(self, name: str, _dir: str, upload_url:bool=False) -> bool | str:
        '''Enter button'''
        try:
            logger.info(f'Enter {path.join(_dir, name)}')
            coord = self.opencv_element(name_element=path.join(_dir, name))
            ...
            if upload_url:
                ...
                self.stay_loade_page()
            return True
        except Exception:
            return False


    def enter_write_edittext(self, name:str, _dir:str, text:str) -> bool:
        '''Enter edittext and write text'''
        try:
            self.enter_button(name=name, _dir=_dir)
    ...
    def open_url(self, url: str, select_url:bool=False, backspace:bool=False, open_tab: bool=False) -> bool:
        '''Open new url'''
        try:
            if open_tab:
                self.stay_loade_page()
